...
def Pred_prob(par1,par2,img,epo1,epo2):
  ...
  for i in range (16):
    cm2[64*i:64*(i+1),64*i:64*(i+1)]=cm
  for i in range (epo1,epo2,1):
    ...
    pred1=torch.tensor(probs).float().reshape(16,64)[:16,:N_map].permute(1,0).reshape(-1)
...
def Pred_shot(par1,par2,img,epo1,epo2):
  ...
  for i in range (epo1,epo2,1):
    ...
    pred1=Convert_Pro(counts).reshape(16,64)[:16,:N_map].permute(1,0).reshape(-1)
    pred2=par2[-1:]*torch.matmul(par2[:-1].reshape(10,N_map*N_pix1*N_pix1),pred1)
    dig=torch.argmax(pred2,dim=0).tolist()
    pdig.append(dig)
  return pdig
